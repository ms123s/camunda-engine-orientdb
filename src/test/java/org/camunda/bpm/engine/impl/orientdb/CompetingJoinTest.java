  ...
  public class SignalThread extends ControllableThread {
    ...
    public synchronized void startAndWaitUntilControlIsReturned() {
      ...
      super.startAndWaitUntilControlIsReturned();
		...
		public void run() {
			try {
				LOG.info("##########################Run:"+this.executionId);
				processEngineConfiguration .getCommandExecutorTxRequired() .execute(new ControlledCommand(activeThread, new SignalCmd(executionId, null, null,null)));

			} catch (OptimisticLockingException e) {
				this.exception = e;
				LOG.info("##########################Exception:"+this.executionId+"/"+e);
			}
			LOG.info("###################Ends:"+this.executionId);
		}
  }
